    ...
    def _type_names(self):
        if isinstance(self.type, tuple):
            return self.join_names(
                [t.__name__.replace("NoneType", "None") for t in self.type]
            )
        else:
...
class ListOfValidator(Validator):
    ...
    def check(self, value):
        return isinstance(value, (tuple, list)) and not filter(
            lambda x: not isinstance(x, self.type), value
        )
...
class ComplexValidator(Validator):
    ...
    @staticmethod
    def can_validate(obj):
        return isinstance(obj, tuple) and not filter(
            lambda x: Validator.get(x) is None, obj
        )
...
def preserve_signature(func):
    ...
    constants = [
        c for c in (getargspec(func)[3] or []) if isinstance(c, GNUTLSConstant)
    ]
...
def _callable_args(*args, **kwargs):
    ...
    for i, arg in enumerate(args):
        ...
        if validator is None:
            raise TypeError(
                "unsupported type `%r' at position %d for argument checking decorator"
                % (arg, i + 1)
            )
        ...
        def check_args(*func_args):
            ...
            for validator in validators:
                if not validator.check(func_args[pos]):
                    raise TypeError(
                        "argument %d must be %s" % (pos + 1 - start, validator.name)
                    )
                pos += 1
